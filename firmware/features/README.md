# Features

Community firmware feature guides:

* [Variables](variables.md)
* [Math](math.md)
* [O Codes](../supported-commands/o-codes.md)
* [Flex Compensation System](flex-compensation-system.md)
* [Jog Modes](jog-modes.md)
* [Custom Tool Setter Position](custom-tool-setter-position.md)
* [Custom Tool Slots](custom-tool-slots.md)
* [LED Behaviour](led-behavior.md)
* [Spindle maximum RPM](spindle-max-rpm.md)
* [USB serial baud rate](usb-serial-baud-rate.md)
* [Spindle Control Types](spindle-control-types/README.md)
  * [C1 Improved PID Spindle Control](spindle-control-types/c1-improved-pid-spindle-control.md)
  * [VESC USB Spindle](spindle-control-types/vesc-usb-spindle.md)
* [Communication Protocol](communication-protocol.md)
* [WiFi AP Auto-Disable](wifi-ap-auto-disable.md)
* [Debug Mode](debug-mode.md)
* [Hardware Health Checks](hardware-health-checks.md)
* [3D Probe Support](../supported-commands/mcodes/probing/3d-probe-support.md)
